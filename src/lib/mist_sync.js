...
class MistSync extends EventEmitter {
    constructor() {
        super();
    }

    async syncSiteDevices(host, token, site) {
        const timer = ms => new Promise(res => setTimeout(res, ms))
        const delay = 150;
        await timer(delay);
        SyncDevices.devices(host, token, site, this);
    }

    siteStats(host, token, site, cb) {
        Sites.stats(host, token, site.id, (err, data) => {
    ...
    async _sync_sites(host, token, sites_from_mist, filtered_site_ids, ids_to_do, index) {
        const timer = ms => new Promise(res => setTimeout(res, ms))
        const delay = 100;
        for (var site_num in sites_from_mist) {
            var site = sites_from_mist[site_num];
            if (filtered_site_ids.length == 0 || filtered_site_ids.includes(site.id)) {
                if (ids_to_do.ids_to_add.includes(site.id)) {
                    this.siteStats(host, token, site, (site, stats) => {
                        index += 1;
                        stats.index = index;
                        ...
                        SiteModel(stats).save((err, res) => {
                            if (err) logger.error(err);
                                ...
                                this.emit('site_added', stats);
                                this.syncSiteDevices(host, token, site);
                            }
                        })
                    })
    ...
    processSites(host, token, sites_from_mist, filtered_site_ids) {
        SiteModel.find({})
            .sort({ index: -1 })
            .exec((err, sites_from_db) => {
                ...
                if (ids_to_do.ids_to_add.length > 0 || ids_to_do.ids_to_update.length > 0) {
                    this._sync_sites(host, token, sites_from_mist, filtered_site_ids, ids_to_do, index)
                }
                    ...
                    sites_from_db.forEach(site => {
                        if (ids_to_do.ids_to_delete.includes(site.id))
                            SiteModel.findOneAndDelete({ site_id: site.id }, (err) => {
                                if (err) logger.error(err);
                                else this.emit('site_removed', site);
                            })
                    })
            })
    }

    processOrg(host, token, org_id) {
        var org = {
            id: org_id,
            name: "",
            stats: {}
        ...
        Orgs.stats(host, token, org_id, (err, data) => {
            if (err) logger.error(err);
                ...
                OrgModel.findOneAndUpdate({ id: org.id }, org, { upsert: true, setDefaultsOnInsert: true }, (err) => {
                    if (err) logger.error(err);
                    this.emit('org_updated', org);
                });
            }
        })
    }

    async sync(host, token, org_id, filtered_site_ids) {
        this.processOrg(host, token, org_id)
        Orgs.sites(host, token, org_id, (err, sites) => {
            if (err) logger.error(err)
            else if (sites && sites.length > 0) {
                this.processSites(host, token, sites, filtered_site_ids);
            }
        })
    }
}

module.exports = MistSync;